...
try {
  ...
  // We'll handle this in the generateWithAI function with mock data
}

// Mock data for development when OpenAI is not available
const mockContentGenerator = (category, topic, difficulty) => {
  console.log(`[DEV MODE] Generating mock content for ${category.name}, difficulty: ${difficulty}`);
  
  return {
    title: `${difficulty.charAt(0).toUpperCase() + difficulty.slice(1)} Tip for ${category.name}`,
    summary: `This is a sample ${difficulty} tip for the ${category.name} category.`,
    body: `# How to Improve Your ${category.name}\n\nThis is a mock content body for testing purposes. In a real app, this would be generated by OpenAI.\n\n- Point 1: Important information\n- Point 2: Follow-up details\n- Point 3: Conclusion\n\nRemember to practice this regularly!`,
    tags: ['sample', category.name.toLowerCase(), difficulty, 'mock-data']
  };
};
...
exports.generateWithAI = async (category, topic, difficulty = 'beginner', customPrompt = null, model = 'gpt-4-turbo-preview') => {
  ...
  // Use mock data in development mode or if OpenAI is not configured
  if (process.env.NODE_ENV === 'development' && (!process.env.OPENAI_API_KEY || !openai)) {
    console.log("Using mock content generator in development mode");
    return mockContentGenerator(category, topic, difficulty);
  }
  
  if (!process.env.OPENAI_API_KEY || !openai) {
    throw new AppError('OpenAI API key is not properly configured', 500);
  }
...
`;
  ...
  try {
    ...
    // In development, fall back to mock data if OpenAI call fails
    if (process.env.NODE_ENV === 'development') {
      console.log("Falling back to mock content after OpenAI API error");
      return mockContentGenerator(category, topic, difficulty);
    }
...
exports.generateMultipleContent = async (category, user, contentType, count = 10, difficulty = 'beginner', model = 'gpt-4-turbo-preview') => {
  ...
  try {
    ...
    promptStrategy = {
      promptText: null, // Will use default prompt in generateWithAI
      isSingle: true,
      count: count,
